...
class PositionCalculatorTest {
    ...
    @BeforeEach
    void setUp() {
        when(exchangeRates.find("USD", BUY_TRADE_DATE.toLocalDate()))
                .thenReturn(new ExchangeRate(BUY_TRADE_DATE.toLocalDate(), "USD", BUY_RATE));
        when(exchangeRates.find("USD", SELL_TRADE_DATE.toLocalDate()))
                .thenReturn(new ExchangeRate(SELL_TRADE_DATE.toLocalDate(), "USD", SELL_RATE));
        when(exchangeRates.find("EUR", BUY_TRADE_DATE.toLocalDate()))
                .thenReturn(new ExchangeRate(BUY_TRADE_DATE.toLocalDate(), "EUR", BUY_RATE));
        when(exchangeRates.find("EUR", SELL_TRADE_DATE.toLocalDate()))
                .thenReturn(new ExchangeRate(SELL_TRADE_DATE.toLocalDate(), "EUR", SELL_RATE));
    }

    @Test
    void shouldCalculateProfit() {
        var buy = unitTrade(TradeOperation.BUY, "100", "0.50", BUY_TRADE_DATE);
        var sell = unitTrade(TradeOperation.SELL, "150", "0.50", SELL_TRADE_DATE);
    ...
    void shouldUseDifferentRatesForTradeAndCommissionCurrencies() {
        ...
        when(exchangeRates.find("EUR", BUY_TRADE_DATE.toLocalDate()))
                .thenReturn(new ExchangeRate(BUY_TRADE_DATE.toLocalDate(), "EUR", eurBuyRate));
        when(exchangeRates.find("EUR", SELL_TRADE_DATE.toLocalDate()))
                .thenReturn(new ExchangeRate(SELL_TRADE_DATE.toLocalDate(), "EUR", eurSellRate));

        var buy = unitTrade(TradeOperation.BUY, "100", "0.50", "USD", "EUR", BUY_TRADE_DATE);
        var sell = unitTrade(TradeOperation.SELL, "150", "0.50", "USD", "EUR", SELL_TRADE_DATE);
    ...
    void shouldCalculateWithPrecision() {
        ...
        when(exchangeRates.find("USD", BUY_TRADE_DATE.toLocalDate()))
                .thenReturn(new ExchangeRate(BUY_TRADE_DATE.toLocalDate(), "USD", usdBuyRate));
        when(exchangeRates.find("USD", SELL_TRADE_DATE.toLocalDate()))
                .thenReturn(new ExchangeRate(SELL_TRADE_DATE.toLocalDate(), "USD", usdSellRate));
        when(exchangeRates.find("EUR", BUY_TRADE_DATE.toLocalDate()))
                .thenReturn(new ExchangeRate(BUY_TRADE_DATE.toLocalDate(), "EUR", eurBuyRate));
        when(exchangeRates.find("EUR", SELL_TRADE_DATE.toLocalDate()))
                .thenReturn(new ExchangeRate(SELL_TRADE_DATE.toLocalDate(), "EUR", eurSellRate));

        var buy = unitTrade(TradeOperation.BUY, "185.13", "1.07", "USD", "EUR", BUY_TRADE_DATE);
        var sell = unitTrade(TradeOperation.SELL, "210.17", "1.23", "USD", "EUR", SELL_TRADE_DATE);
    ...
    private static UnitTrade unitTrade(TradeOperation op, String price, String commission,
        ...
        return new UnitTrade(op, new BigDecimal(price), new BigDecimal(commission), "USD", "EUR", tradeDate);
    }

    private static UnitTrade unitTrade(TradeOperation op, String price, String commission,
                                       String currency, String commCurrency, LocalDateTime tradeDate) {
        return new UnitTrade(op, new BigDecimal(price), new BigDecimal(commission),
                currency, commCurrency, tradeDate);
    }
}
